...
class ppo2:
    ...
    class Model(object):
        def __init__(self, *, policy, ob_space, ac_space, nbatch_act, nbatch_train,
            ...
            #tf.summary.scalar('CLIPRANGE',CLIPRANGE)
            # writer = tf.summary.FileWriter('tmp/',sess)
            ...
            self.restores = []
            self.restores.append(get_restore('cpt/checkpoints/all2'))
    ...
    def learn(self, sess):
        ...
        for update in range(1, nupdates+1):
            ...
            # writer = tf.summary.FileWriter('tmp/',sess.graph)
            # result = sess.run('model/v/b:0')
            # writer.add_summary(result,update)
